...
module JekyllImport
  ...
  module WordpressDotCom
    attr :image_folder
    @image_folder = 'assets/images/squarespace_images'
    ...
    def self.process(filename = {:source => "Squarespace-Wordpress-Export-03-16-2019.xml"})
      ...
      (doc/:channel/:item).each do |item|
        ...
        doc.css('img').each do |element|
          ...
          if u.host.end_with? 'squarespace.com'
            # filename = u.path.sub(/^\//, '').gsub('/', '_') + '_' + (u.fragment || '') + '?format=2500w'
            filename = u.path.sub(/^\//, '').gsub('/', '_')
